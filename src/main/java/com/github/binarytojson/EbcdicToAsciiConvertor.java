...
public class EbcdicToAsciiConvertor {
    ...
    private static final List<PrimitiveType> FILL_AND_SEG_NAME = Arrays.asList(
            PrimitiveType.builder().name(FILL_KEY).dataType(DataType.CHAR).level(1).length(6).build(),
            PrimitiveType.builder().name(SEG_NAME_KEY).dataType(DataType.CHAR).level(1).length(8).build());

    private final BufferedInputStreamReader reader = new BufferedInputStreamReader();

    public void convert(InputStream inputStream, OutputStream os, List<HeaderRecordDto> headers,
                        GenerationType generationType, Cache cache, Mode... modes) throws IOException {
        Iterable<byte[]> records = reader.readBinaryFile(new BufferedInputStream(inputStream, cache.getBufferSize()));
        ...
        try (Writer writer = generationType.getWriterFactory().create(os)) {
            ...
            String headerName = headers.stream()
                    .filter(it -> it.getPrimitiveTypes().get(0).isRootElement()).findFirst()
                    .map(it -> it.getPrimitiveTypes().get(0).getName()).orElse(null);
            List<HeaderRecordDto> headersWithoutRoot = headers
                    .stream().filter(it -> !it.getPrimitiveTypes().get(0).isRootElement()).collect(Collectors.toList());
            for (byte[] bytes : records) {
                HeaderRecordDto headerRecord = getHeaderRecordDto(headersWithoutRoot, index, bytes);
                List<PrimitiveType> primitiveTypes = headerRecord.getPrimitiveTypes().stream().map(PrimitiveType::copy)
                ...
                if (HEADER_NAME_SKIP.equals(headersWithoutRoot.get(0).getPrimitiveTypes().get(0).getName())) {
                    ...
                    structureRecords.add(structureRecord);
                    groupIndex = updateGroupIndex(headersWithoutRoot, headerName, mode,
                            index, groupIndex, writer, structureRecords);
                }
                index++;
            }
    ...
    int updateGroupIndex(List<HeaderRecordDto> headers, String rootName, Mode mode, int index,
                         int groupIndex, Writer writer, List<StructureRecord> structureRecords) {
    ...
    private void writeGroup(Mode mode, List<StructureRecord> structureRecords, int groupIndex, Writer writer) {
        StructureRecord structureRecordGroup = new StructureRecord(
                convertToByteArray(structureRecords.stream().map(structureRecord ->
                        new AbstractMap.SimpleEntry<>(structureRecord.getBytes(),
                                calculateFixedLength(structureRecord.getTypes()))).collect(Collectors.toList())),
                structureRecords.stream().flatMap(it -> it.getTypes().stream()).collect(Collectors.toList()));
        if (groupIndex == 0) {
    ...
    private HeaderRecordDto getHeaderRecordDto(List<HeaderRecordDto> headers, int index, byte[] bytes) {
        if (HEADER_NAME_SKIP.equals(headers.get(0).getPrimitiveTypes().get(0).getName())) {
            ...
            Map<String, Object> map = structureRecord.processList(FILL_AND_SEG_NAME, null, Mode.WITHOUT_ARRAY);
            Optional<HeaderRecordDto> foundHeader = headers.stream()
                    .filter(it -> Objects.equals(map.get(SEG_NAME_KEY), it.getPrimitiveTypes().get(0).getName())).findFirst();
